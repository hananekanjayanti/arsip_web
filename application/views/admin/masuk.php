							...
							<table cellspacing="0" class="display nowrap table table-hover table-striped table-bordered tableku" width="100%">
								...
								<tbody id="isi">
									...
									<tr>
										...
										<td>
											<button class="btn btn-outline-success" data-toggle="modal" data-target="#update<?php echo $key->id ?>"><i data-toggle="tooltip" title="Update masuk" class="fa fa-edit"></i></button>
											<a href="<?php echo base_url('admin/detmasuk/'.$key->id) ?>" class="btn btn-outline-primary" data-toggle="tooltip" title="Detail Surat Masuk"><i class="fa fa-search"></i></a>
											<a href="<?php echo base_url('admin/delmasuk/'.$key->id) ?>" class="btn btn-outline-danger" data-toggle="tooltip" onclick="return confirm('Hapus surat masuk?')" title="Hapus Surat Masuk"><i class="fa fa-trash"></i></a>
...
<div class="modal fade text-xs-left" id="update<?php echo $e->id ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			...
			<form method="post" action="<?php echo base_url('admin/updatemasuk/'.$e->id) ?>" enctype="multipart/form-data">
...
<script>
	...
	function carinosurat2(id) {
		if ($("#no_surat"+id.toString()).val()!=='') {
			$.ajax({
				type: "GET",
				url: "<?php echo base_url('admin/carinosurat/') ?>"+$("#no_surat"+id.toString()).val(),
				success: function (data) {
					var dataa = data;
					if (dataa==1) {
