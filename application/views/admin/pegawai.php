									...
									<tr>
										...
										<td>
											...
											<?php if ($key->akses!=="Admin"): ?>
												<a href="<?php echo base_url('admin/delpegawai/'.$key->id) ?>" onclick="return confirm('Hapus pegawai? semua data yang berhubungan dengan pegawai ini akan terhapus!')" class="btn btn-outline-danger" data-toggle="tooltip" title="Hapus pegawai"><i class="fa fa-trash"></i></a>
											<?php endif ?>
										</td>
									</tr>
								<?php endforeach ?>
	...
	<div class="modal fade text-xs-left" id="update<?php echo $key->id ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				...
				<form method="post" action="<?php echo base_url('admin/updpegawai/'.$key->id) ?>">
									...
									<div class="col-md-6">
										...
										<div class="form-group">
											...
											<select name="akses" required="" class="form-control">
												...
												<option <?php if($key->akses=="Sekretariat"){echo "selected";} ?> value="Sekretariat">Sekretariat</option>
											</select>
										</div>
									</div>
									<div class="col-md-6">
										...
										<div class="row">
											...
											<div class="col-md-6">
												<div class="form-group">
													...
													<div class="position-relative has-icon-right">
														<input type="password" value="<?php echo $key->password ?>" id="lama3<?php echo $key->id ?>" required="" class="form-control" name="password">
														<div class="form-control-position">
															<i class="icon-eye4"  data-toggle="tooltip" data-placement="top" onmousedown="liat4(<?php echo $key->id ?>)" onmouseup="tutup4(<?php echo $key->id ?>)"></i>
														</div>
													</div>
												</div>
											</div>
											...
											<div class="col-md-6">
												...
												<div class="form-group">
													...
													<div class="position-relative has-icon-right">
														...
														<div class="form-control-position">
															<i class="icon-eye4"  data-toggle="tooltip" data-placement="top" onmousedown="liat3()" onmouseup="tutup3()"></i>
														</div>
													</div>
												</div>
											</div>
	...
	<script>
		function carinip() {
			if ($("#nip").val()!=='') {
				$.ajax({
					type: "GET",
					url: "<?php echo base_url('admin/carinip/') ?>"+$("#nip").val(),
		...
		function cariusernamepegawai() {
			if ($("#usernamebaru").val()!=='') {
				$.ajax({
					type: "GET",
					url: "<?php echo base_url('admin/cariusername/') ?>"+$("#usernamebaru").val(),
					success: function (data) {
						var dataa = data;
						if (dataa==1) {
							$("#btnnip").prop('disabled',true);
		...
		function carinip2(id) {
			if ($("#nip"+id.toString()).val()!=='') {
				$.ajax({
					type: "GET",
					url: "<?php echo base_url('admin/carinip/') ?>"+$("#nip"+id.toString()).val(),
					success: function (data) {
						var dataa = data;
						if (dataa==1) {
		...
		function cariusernamepegawai2(id) {
			if ($("#usernamebaru"+id.toString()).val()!=='') {
				$.ajax({
					type: "GET",
					url: "<?php echo base_url('admin/cariusername/') ?>"+$("#usernamebaru"+id.toString()).val(),
					success: function (data) {
						var dataa = data;
						if (dataa==1) {
							$("#btnnip"+id.toString()).prop('disabled',true);
