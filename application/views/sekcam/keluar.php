							...
							<table cellspacing="0" class="display nowrap table table-hover table-striped table-bordered tableku" width="100%">
								...
								<tbody id="isi">
									...
									<tr>
										...
										<td>
											<a href="<?php echo base_url('sekcam/detkeluar/'.$key->id) ?>" class="btn btn-outline-primary" data-toggle="tooltip" title="Detail Surat keluar"><i class="fa fa-search"></i></a>
	...
	<div class="modal fade text-xs-left" id="update<?php echo $e->id ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				...
				<form method="post" action="<?php echo base_url('sekcam/updatekeluar/'.$e->id) ?>" enctype="multipart/form-data">
...
<div class="modal fade text-xs-left" id="tambah" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			...
			<form method="post" action="<?php echo base_url('sekcam/addkeluar') ?>" enctype="multipart/form-data">
				<div class="modal-body">
					<div class="row">
						<div class="col-md-6">
							<div class="row">
...
<script>
	function carinosurat() {
		if ($("#no_surat").val()!=='') {
			$.ajax({
				type: "GET",
				url: "<?php echo base_url('sekcam/carinosurat_keluar/') ?>"+$("#no_surat").val(),
	...
	function carinosurat2(id) {
		if ($("#no_surat"+id.toString()).val()!=='') {
			$.ajax({
				type: "GET",
				url: "<?php echo base_url('sekcam/carinosurat_keluar/') ?>"+$("#no_surat"+id.toString()).val(),
				success: function (data) {
					var dataa = data;
					if (dataa==1) {
